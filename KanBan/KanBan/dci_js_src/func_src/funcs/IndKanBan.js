...
function logout(postvalue, uid) {
	if (navigator.userAgent.match("Firefox") == "Firefox") {
		...
		if (ajax) {
			ajax.open("GET", '/KanBan/Login.dsc?action=logout&uid=' + uid + '&DCITag=' + postvalue, false);
			ajax.send("");
		}
...
function setLightSize(FUNCID,GRID,value){
	var getSetHeight = parseInt(value);
	getSetHeight = getSetHeight <= 28 ? 16 : getSetHeight-12;
	var getGridColumn = GRID.getView().getHeaderCt().getGridColumns();
	for ( var i = 0; i < getGridColumn.length; i++) {
		if(getGridColumn[i].xtype != undefined && getGridColumn[i].xtype == "dcilightcolumn"){
			getGridColumn[i].rowHeight = getSetHeight;
			for ( var j = 0; j < GRID.getStore().getCount(); j++) {
				 document.getElementById("light"+FUNCID+j+";"+i).style.height = getSetHeight+"px" ;
				 document.getElementById("light"+FUNCID+j+";"+i).style.width = getSetHeight+"px" ;
			}
		}
	}
}
...
function changeKanbanGrid(newValue, classname, changeType, GRID) {
	try {
		var pName = null;
		var pValue = null;
		...
		if (changeType == "FS") {
			...
			setLightSize(classname.substring(0,classname.length-4),GRID,newValue);
		...
		if (records != null && records.length == 1) {
			...
			Ext.EventManager.on(window, 'unload', function() {
				if (navigator.userAgent.match("Firefox") == "Firefox") {
					var ajax = new XMLHttpRequest();
					if (ajax) {
						ajax.open("GET", './../../Login.dsc?action=logout&uid=' + globeuid + '&DCITag=' + postvalue, false);
						ajax.send("");
					}
				...
				listeners : {
					exception : function(proxy, response, operation, eOpts) {
						if (response.responseText.indexOf('@dcifiltererrtag@') != -1) {
							...
							if (bodygrid && bodygrid != null) {
								Ext.Msg.alert(langs.errmsg, response.responseText, function() {
									bodygrid.setLoading(false);
								});
							} else {
								Ext.Msg.alert(langs.errmsg, response.responseText);
							}
						}
					}
				}
		...
		var hp2 = Ext.create('Ext.panel.Panel', {
							...
							items : [ {
								...
								listeners : {
									change : function(slider, newValue, thumb, eOpts) {
										...
										changeKanbanGrid(newValue, gridClass, "FS", bodygrid);
								...
								listeners : {
									change : function(picker, newValue, oldValue, eOpts) {
										...
										changeKanbanGrid(newValue, gridClass, "FC", bodygrid);
										params.fc = newValue;
									}
								}
							}, {
								...
								listeners : {
									change : function(slider, newValue, thumb, eOpts) {
										...
										changeKanbanGrid(newValue, gridClass, "RH", bodygrid);
										params.rh = newValue;
									},
									afterrender : function(slider, eOpts) {
										var me = this;
										...
										items : [ {
											...
											listeners : {
												change : function(picker, newValue, oldValue, eOpts) {
													...
													changeKanbanGrid(newValue, gridClass, "BC", bodygrid);
													params.bc = newValue;
												}
											}
										} ]
										...
										items : [ {
											...
											listeners : {
												change : function(picker, newValue, oldValue, eOpts) {
													...
													changeKanbanGrid(newValue, gridClass, "BcEven", bodygrid);
			...
			setCss : function(displayInfo) {
				...
				if (displayInfo.grid_row_height != null && displayInfo.grid_row_height != "") {
					...
					setLightSize(gridClass.substring(0,gridClass.length-4),bodygrid,displayInfo.grid_row_height);
				} else {
					cssText += "height:28px;";
					...
					setLightSize(gridClass.substring(0,gridClass.length-4),bodygrid,"16");
				}
			...
			if (record.length > 0) {
				if (gridStore != null) {
					...
					if (bodygrid != null) {
						...
						var getSetHeight = "16";
						if (displayInfo.grid_row_height != null && displayInfo.grid_row_height != "") {
							getSetHeight = displayInfo.grid_row_height;
							getSetHeight = getSetHeight <= 28 ? 16 : getSetHeight-12
						}
						var getGridColumn = record[0].get('cols');
						for ( var i = 0; i < getGridColumn.length; i++) {
							if(getGridColumn[i].xtype != undefined && getGridColumn[i].xtype == "dcilightcolumn"){
								getGridColumn[i].rowHeight = getSetHeight;
							}
